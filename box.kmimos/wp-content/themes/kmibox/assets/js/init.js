$(function($){
	...
	$('section').on('click','[data-action="sumarItems"]', function(){
		...
		var total = $('[data-type="primary"]').val() * nItemsExtras;
...
//			obj.find('#estatus').val( datos[id]['estatus'] );
	...
	.on('success.form.bv', function(e) {
		...
		$.post( urlbase+"/ajax/register.php", {
			...
			donde_conociste: $('[name="donde_conociste"]').val(),
			...
			dir_ciudad: $('#dir_ciudad').val(),
			dir_colonia: $('[name="dir_colonia"]').val(),
			dir_codigo_postal: $('[name="dir_cod_postal"]').val()
